...
class Utils {
    ...
    static function getDebitOperationName(){
        return 'DEBIT';
    }

    static function getCreditOprationName(){
        return 'CREDIT';
    }

    static function getInitialDepositDescription(){
        return 'INITIAL DEPOSIT';
    }
    
    
    static function getUploadImageExtensions(): array
    {
        return [".png", ".jpg", ".jpeg", ".PNG", ".JPG", ".JPEG"];
    }

    static function getUploadMaxSize($onlyNumbers = true): int|string 
    {
        if(!$onlyNumbers){
            return '1024k';
        }
    ...
    static function getAbbreviatedAmount($number, $floating_points = 1){
        if(!$number){
            return 0;
        }
    ...
    static function getImageMimeTypes():array
    {
        return [
            'image/jpeg',
            'image/png',
    ...
    static function getParametersReorganised(array $params){
        $return = [];
        //dd($params);
        if(!empty($params)){
            foreach($params as $param){
                $return[$param->getLabel()] = $param->getValue();
            }
        }
        return $return;
    }
    ...
    static function getArrayAsHTMLAttributes(array $attributes):string
    {
        $attributesString = "";
        if(!empty($attributes)){
    ...
    static function getTeamsForSelect(array $teams): array {
        $return = [''=>''];
        if(!empty($teams)){
    ...
    static function getCountriesForSelect(): array {
        $return = [''=>''];
        $countries = self::getAllCountries();
        if(!empty($countries)){
            foreach($countries as $code => $countryElement){
                $return[$countryElement['name']] = $code;
            }
        }
        return $return;
    }

    static function getPlayersDataFromReorganisedTeamData(array $reorganisedTeamsArray, $limit=0):array {
        $return = [];

        if(!empty($reorganisedTeamsArray)){
            //dd($reorganisedTeamsArray);
            $counter = 0;
            foreach($reorganisedTeamsArray as $key=> $team){
    ...
    static function getReorganisedTeamPageData(array $teams): array {
        $reorganisedTeamData = [];
        if(!empty($teams)){
            foreach ($teams as $key => $team){
                //IF NOT EXISTING
                if(!isset($reorganisedTeamData[$team['team_id']])){
                    $reorganisedTeamData[$team['team_id']]=[
                        'id'=>$team['team_id'],
                        'name'=>$team['team_name'],
                        ...
                        'players'=> !$team['player_id'] ? [] : [
                            [
                                ...
                                'country'=>$team['player_country'],
                                'created_at'=>$team['player_created_at'],
                                'fullname'=>$team['player_first_name']." ".$team['player_last_name'],
                            ]
                        ]
                    ];
                }else{
                    ...
                    $reorganisedTeamData[$team['team_id']]['players'][] = [
                        'id'=>$team['player_id'],
                        'photo'=>$team['photo'],
                        'surname'=>$team['surname'],
                        'country'=>$team['player_country'],
        ...
        //dd($reorganisedTeamData);
        return $reorganisedTeamData;
    }

    static function getAllCountries(string $byCountryCode = ''): array {
        ...
        $countries = [
            ...
            if(!!$byCountryCode){
                return self::maybeNullOrEmpty($countries, $byCountryCode, null);
            }
            return $countries;
        }
}
